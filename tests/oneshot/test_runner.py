...
def test_simple_run(all_agents):
    ...
    for config in runner.existing_config_names:
        assert len(runner.agents_of(EqualDistOneShotAgent, config=config)) == len(runner.agents_of(RandDistOneShotAgent, config=config))
...
def test_copying():
    r1 = WorldRunner(ANACOneShotContext(n_steps=10), n_configs=3, n_repetitions=2, save_worlds=True)
    ...
    for i, (c1, c2) in enumerate(((r1.configs, r2.configs), (r1.configs, r3.configs))):
        assert len(c1) == len(c2), f"Error in comparing {'r1, r2' if i == 0 else 'r1, r3'}"
        for a, b in zip(c1, c2, strict=True):
            for k in ("agent_types", "agent_params"):
                assert len(a) == len(b), f"Error in comparing {k} for {'r1, r2' if i == 0 else 'r1, r3'}"
...
def test_std_runner(all_agents):
    ...
    for config in runner.existing_config_names:
        assert len(runner.agents_of(GreedyStdAgent, config=config)) == len(runner.agents_of(RandomStdAgent, config=config))
    score_summary = runner.score_summary()
    assert len(score_summary) == 2, f"{score_summary}"
    if not all_agents:
        assert len(runner.scores) == 3 * 2 * 2
...
def test_runner_with_call(progress):
    runner = WorldRunner(ANACOneShotContext(name="myc", n_steps=20), n_configs=3, n_repetitions=2)
