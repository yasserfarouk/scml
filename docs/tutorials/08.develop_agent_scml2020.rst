...
.. container::
   ...
   ::
      ...
      <strong>Deprecation Notice:</strong> SCML 2020 and 2019 worlds are now deprecated. You should not use them for any future development. This tutorial is only kept for understanding legacy agents developed for the ANAC SCML competition's standard and collusion tracks between 2019 and 2023.
...
.. code:: python
   ...
   def create_ufun(self, is_seller, prange, issues):
       ...
       return MappingUtilityFunction(
           lambda x: (
               -1000 if x[UNIT_PRICE] > self.prices[0] else prange[1] - x[UNIT_PRICE]
           ),
           reserved_value=0.0,
           issues=issues,
       )
