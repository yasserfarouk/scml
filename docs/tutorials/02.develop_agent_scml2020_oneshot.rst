...
.. raw:: html
    ...
    <table border="1" class="dataframe">
      ...
      <tbody>
        <tr>
          <th rowspan="7" valign="top">01Ran@0</th>
          <th>04MDN@1</th>
          <td>6.000000</td>
          <td>9.000000</td>
        </tr>
...
::
    ...
    <ipython-input-26-af3427efb9b3> in <module>
         40         return d
         41
    ---> 42 world, ascors, tscores = try_agent(GreedyIndNeg)
         43
         44 print_type_scores(tscores)


    <ipython-input-3-6fe9c6f702a8> in try_agent(agent_type, n_processes)
          8 def try_agent(agent_type, n_processes=2):
          9     """Runs an agent in a world simulation against a randomly behaving agent"""
    ---> 10     return try_agents([RandomOneShotAgent, agent_type], n_processes)
         11
         12 def try_agents(agent_types, n_processes=2, n_trials=1, draw=True, agent_params=None):


    <ipython-input-3-6fe9c6f702a8> in try_agents(agent_types, n_processes, n_trials, draw, agent_params)
         25         construct_graphs=True,
         26         )
    ---> 27         world.run()
         28
         29         all_scores = world.scores()


    ~/code/projects/negmas/negmas/situated/situated.py in run(self)
       3538             if self.time >= self.time_limit:
       3539                 break
    -> 3540             if not self.step():
       3541                 break
       3542


    ~/code/projects/negmas/negmas/situated/situated.py in step(self)
       3140             for priority in sorted(self._entities.keys()):
       3141                 for agent in self._entities[priority]:
    -> 3142                     self.call(agent, agent.init_)
       3143                     if self.time >= self.time_limit:
       3144                         return False


    ~/code/projects/negmas/negmas/situated/situated.py in call(self, agent, method, *args, **kwargs)
       3117             self.on_exception(agent, e)
       3118             if not self.ignore_agent_exception:
    -> 3119                 raise e
       3120             exc_type, exc_value, exc_traceback = sys.exc_info()
       3121             self.logerror(


    ~/code/projects/negmas/negmas/situated/situated.py in call(self, agent, method, *args, **kwargs)
       3101         try:
       3102             _strt = time.perf_counter()
    -> 3103             result = method(*args, **kwargs)
       3104             _end = time.perf_counter()
       3105             if self.disable_agent_printing:


    ~/code/projects/scml/src/scml/oneshot/sysagents.py in init_(self)
         56             if not self.ufun:
         57                 self.make_ufun(add_exogenous=True)
    ---> 58         super().init_()
         59
         60     def init(self):


    ~/code/projects/negmas/negmas/situated/situated.py in init_(self)
        452         self._initialized = True
        453         self.__current_step = 0
    --> 454         self.init()
        455
        456     def step_(self):


    ~/code/projects/scml/src/scml/oneshot/sysagents.py in init(self)
         63         else:
         64             self._obj._awi = AWIHelper(self)
    ---> 65         super().init()
         66
         67     def before_step(self):


    ~/code/projects/negmas/negmas/situated/situated.py in init(self)
       2078     def init(self):
       2079         """Override this method to modify initialization logic"""
    -> 2080         self._obj.init()
       2081
       2082     def step(self):


    ~/code/projects/scml/src/scml/oneshot/agent.py in init(self)
        549     def init(self):
        550         super().init()
    --> 551         self._ufuns = self._get_ufuns()
        552
        553     def step(self):


    ~/code/projects/scml/src/scml/oneshot/agent.py in _get_ufuns(self)
        518         meaningful value and that the ufun is normalized if needed
        519         """
    --> 520         ufuns = self.generate_ufuns()
        521         if not self._normalize and not self._set_reservation:
        522             return ufuns


    <ipython-input-26-af3427efb9b3> in generate_ufuns(self)
          7             if self.awi.is_system(partner_id):
          8                 continue
    ----> 9             d[partner_id] = LinearAdditiveUtilityFunction(
         10                 dict(
         11                     quantity=lambda x: 0.1 * x,


    ~/code/projects/negmas/negmas/preferences/linear.py in __init__(self, values, weights, bias, *args, **kwargs)
        517         if isinstance(values, dict):
        518             if self.issues is None:
    --> 519                 raise ValueError(
        520                     "Must specify issues when passing `issue_utilties` or `weights` is a dict"
        521                 )


    ValueError: Must specify issues when passing `issue_utilties` or `weights` is a dict
