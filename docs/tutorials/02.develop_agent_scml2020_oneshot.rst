...
.. image:: 02.develop_agent_scml2020_oneshot_files/02.develop_agent_scml2020_oneshot_3_1.png
...
-  Depending on your base-class, you will also need to implement methods
   ...
   -  **OneShotPolicy** If your agent is based on ``OneShotPolicy``, you
      will get an ``act()`` call when you need to respond to offers from
      your partners or you need to pass your initial offers to them. You
      will also have the change to adjust your state and action in
      ``encode_state()`` and ``decode_action()`` called before and after
      ``act()`` respectively.
...
Probably the easiest way to build your agent in SCML 2024 oneshot is to
use the ``OneShotPolicy`` base class. This is an example of an agent
that behaves randomly:
...
.. image:: 02.develop_agent_scml2020_oneshot_files/02.develop_agent_scml2020_oneshot_13_1.png


The default state received by the ``act()`` function is a
``OneShotState`` which includes lots of information about the current
state of the market at the point in which the agent is asked to act. The
state contains a ``current_nmis`` dictionary that maps partner IDs to
the ``SAONMI`` objects that can be used to access information about the
current negotaition with the corresponding partner including the
``state`` and ``current_offer``. If our agent is the first agent to
offer, these offers will be ``None`` for all partners.

The agent should return a dictionary mapping each partner (in the
``current_nmis`` keys) to a response. A single response is an object of
type ``SAOResponse`` which is a tuple with a ``ResponseType``
(``END_NEGOTIATION``, ``ACCEPT_OFFER``, or ``REJECT_OFFER``) and an
outcome (only used when the response-type is ``REJECT_OFFER``).
...
Out of the remaining 98% of cases, it will accept the offer with
probability 5%:
...
Otherwise, it will simply return a random outcome which is sampled using
the ``SAONMI``
...
Let’s build a more useful policy and showcase the use of
``encode_state`` and ``decode_action`` methods:
...
This is the base class of all agents for SCML-OneShot.
``OneShotPolicy``, ``SyncOneShotAgent`` and
``SingleAgreementOneShotAgent`` inherit from this class and provide
support for a simplified way of developing your agent (or so we think).
It is perfectly OK to use ``OneShotAgent`` directly as the base of your
agent.
...
**Note that this agent is not effective since 2023 because the price
range is too small that it really does not matter much which price is
...
.. container::
   ...
   This section is not necessary for SCML-OneShot 2024 competition
