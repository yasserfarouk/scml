...
from .common import Context, isin
...
@define(frozen=True)
class WorldContext(Context, ABC):
    """A context that generates oneshot worlds with a single agent of type `agent_type` with predetermined structure and settings"""
...
@define(frozen=True)
class FixedPartnerNumbersContext(WorldContext):
    """Generates a world fixing the agent level, production capacity and the number of suppliers, consumers, and optionally same-level competitors."""
    ...
    def make_world(
        self,
        world_type: type[SCML2020OneShotWorld],
        types: tuple[type[OneShotAgent], ...] = (OneShotDummyAgent,),
        params: tuple[dict[str, Any], ...] | None = None,
    ) -> SCML2020OneShotWorld:
        """Generates a world"""
    ...
    def is_valid_awi(self, awi: OneShotAWI) -> bool:  # type: ignore
        # find my level
    ...
    def contains_context(self, context: Context) -> bool:
        ...
        if not isinstance(context, self.__class__):
            return False
        if not isin(context.year, self.year):
            return False
        if not isin(context.n_processes, self.n_processes):
            return False
        if not isin(context.level, self.level):
            return False
        if not isin(context.n_consumers, self.n_consumers):
            return False
        if not isin(context.n_suppliers, self.n_suppliers):
            return False
        if not isin(context.n_competitors, self.n_competitors):
            return False
        if not isin(context.n_agents_per_level, self.n_agents_per_level):
            return False
        if not isin(context.n_lines, self.n_lines):
            return False
        if set(context.non_competitors).difference(list(self.non_competitors)):
            return False
        return True
...
@define(frozen=True)
class ANACContext(WorldContext):
    ...
    def make_world(
        ...
        type_ = world_type
        d = self.world_params
        generated = type_.generate(agent_types=types, agent_params=params)
...
class FixedPartnerNumbersOneShotContext(FixedPartnerNumbersContext):
    def make(
        ...
        return super().make_world(type_, types, params)
