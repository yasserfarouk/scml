...
class OneShotEnv(gym.Env):
    def __init__(
        ...
        assert context.contains_context(action_manager.context, raise_on_failure=debug), (
        ...
        assert context.contains_context(observation_manager.context, raise_on_failure=debug), (
        ...
        self._agent_params = agent_params if agent_params is not None else {}
        self._agent_id: str = ""
    ...
    def step(self, action):  # type: ignore
        ...
        if self._skip_after_negotiations:
            while not terminated and len(self._agent.awi.current_states) < 1:
                reward_info = self._reward_function.before_action(self._agent.awi)
                terminated = not self._world.step_with(
                    {self._agent_id: {}}  # type: ignore
                )
                reward += self._reward_function(self._agent.awi, {}, reward_info)
                if self._world.current_step >= self._world.n_steps - 1:
                    terminated = 1
        obs = self._get_obs()
        info = self.calc_info()
