...
from scml.std.common import NegotiationDetails, StdState
from scml.std.rl.common import isin
from scml.std.rl.factory import (
...
class ObservationManager(Protocol):
    ...
    def encode(self, state: StdState) -> np.ndarray:
        """Encodes an observation from the agent's state"""
        ...

    def make_first_observation(self, awi: StdAWI) -> np.ndarray:
        """Creates the initial observation (returned from gym's reset())"""
        ...

    def get_offers(self, awi: StdAWI, encoded: np.ndarray) -> dict[str, SAOResponse]:
...
class FixedPartnerNumbersObservationManager(BaseObservationManager):
    ...
    def __attrs_post_init__(self):
        assert isinstance(self.factory, FixedPartnerNumbersStdFactory)
    ...
    def is_valid(self, env) -> bool:
        """Checks that it is OK to use this observation manager with a given `StdEnv`"""
...
class LimitedPartnerNumbersObservationManager(BaseObservationManager):
    ...
    def __attrs_post_init__(self):
        if isinstance(self.factory, LimitedPartnerNumbersStdFactory):
            object.__setattr__(self, "n_suppliers", self.factory.n_suppliers)
            object.__setattr__(self, "n_consumers", self.factory.n_consumers)
            object.__setattr__(self, "max_quantity", self.factory.n_lines)
            object.__setattr__(self, "n_lines", self.factory.n_lines)
        elif isinstance(self.factory, FixedPartnerNumbersStdFactory):
            object.__setattr__(
                self,
                "n_suppliers",
        ...
        object.__setattr__(
            ...
            "sub_manager",
            FixedPartnerNumbersObservationManager(
                factory=FixedPartnerNumbersStdFactory(
