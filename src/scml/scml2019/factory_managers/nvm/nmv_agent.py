...
class NVMFactoryManager(DoNothingFactoryManager):
    ...
    def __init__(self, name=None, simulator_type: Union[str, Type[FactorySimulator]] = FastFactorySimulator, parameters: Optional[Dict] = None):
        """
        ...
        if parameters is not None:
            self.marginal_calculation_on_sign_contract = parameters['marginal_calculation_on_sign_contract']
    ...
    def on_production_failure(self, failures: List[ProductionFailure]) -> None:
        ...
        if self.verbose:
            print(f'A failure on production occur at {self.awi.current_step}!!! = {[str(fail)  + "**" for fail in failures]}')
    ...
    def on_negotiation_failure(self, partners: List[str], annotation: Dict[str, Any], mechanism: AgentMechanismInterface, state: MechanismState) -> None:
        """Called whenever a negotiation ends without agreement"""
        if self.verbose and annotation["cfp"]["product"] == self.output_index:
            print(f"\tFailed to negotiate about output: q = {annotation['cfp']['quantity']}, p = {annotation['cfp']['unit_price']}, t = {annotation['cfp']['time']}")
        if self.verbose and annotation["cfp"]["product"] == self.input_index:
            print(f"\tFailed to negotiate about input: q = {annotation['cfp']['quantity']}, p = {annotation['cfp']['unit_price']}, t = {annotation['cfp']['time']}")
        if self.verbose and annotation['cfp']['product'] in self.middle_man_products:
            print(f"\tMiddle Man Failed to negotiate about a cfp to buy? {annotation['cfp']['is_buy']} for product {annotation['cfp']['product']}. "
    ...
    def sign_contract(self, contract: Contract) -> Optional[str]:
        ...
        if contract.annotation['cfp'].product == self.input_index or contract.annotation['cfp'].product == self.output_index:
            if self.marginal_calculation_on_sign_contract:
                value = self.agent_brain.marginal_value_contract(current_time=self.current_step,
            ...
            if value >= 0 and contract.annotation['buyer'] == self.id and \
                    contract.annotation['cfp'].product == self.input_index and \
                    self.contracted_buys_at_t[self.input_index][contract.agreement['time']] <= self.limit_number_buys_contracts_at_t and \
                    self.awi.state.storage[self.output_index] <= self.limit_sign_storage and \
                    contract.agreement['time'] <= self.limit_sign_time_input_product:
                sign = True
            elif value >= 0 and contract.annotation['buyer'] != self.id and \
                    contract.annotation['cfp'].product == self.output_index and \
                    self.contracted_sales_at_t[self.output_index][contract.agreement['time']] <= self.limit_number_sales_contracts_at_t and \
                    sum([self.contracted_buys_at_t[self.input_index][t]
                         for t in range(0, contract.agreement['time'] - 1)]) >= contract.agreement['quantity']:
                sign = True
            if sign:
                # print(f'CONTRACT REGISTERED AS SIGNED!: {contract}')
                self.signed_contracts_for_factory[contract.agreement['time']] += [(contract.annotation['buyer'] == self.id,
                                                                                   (contract.agreement['unit_price'],
                                                                                    contract.agreement['quantity'],
                                                                                    contract.agreement['time']))]
        # ---- MIDDLE MAN STUFF
        else:
            if self.middle_man_active and \
                    contract.annotation['cfp'].product in self.middle_man_products and \
                    contract.annotation['buyer'] == self.id and \
                    self.contracted_buys_at_t[contract.annotation['cfp'].product][contract.agreement['time']] <= self.limit_number_buys_contracts_at_t and \
                    self.awi.state.storage[contract.annotation['cfp'].product] <= self.limit_sign_storage and \
                    sum([self.contracted_buys_at_t[self.input_index][t]  # @todo warning. I am trying to limit the amount of stuff we buy, sometimes we get too much stuff.
                         for t in range(max(0, contract.agreement['time'] - 10), contract.agreement['time'] - 1)]) <= self.limit_sign_storage:
                sign = True
            elif self.middle_man_active and \
                    contract.annotation['cfp'].product in self.middle_man_products and \
                    contract.annotation['buyer'] != self.id and \
                    self.awi.state.storage[contract.annotation['cfp'].product] >= contract.agreement['quantity']:
                sign = True
    ...
    def on_contract_signed(self, contract: Contract) -> None:
        ...
        if contract.annotation['buyer'] == self.id:
            if self.verbose:
                print(f"\t\tSigned a buy contract for product {contract.annotation['cfp'].product}: "
                      f"q = {contract.agreement['quantity']}, p = {contract.agreement['unit_price']}, t = {contract.agreement['time']}")
            self.contracted_buys_at_t[contract.annotation['cfp'].product][contract.agreement['time']] += contract.agreement['quantity']
        elif contract.annotation['buyer'] != self.id:
            if self.verbose:
                print(f"\t\tSigned a sell contract for product {contract.annotation['cfp'].product}: "
                      f"q = {contract.agreement['quantity']}, p = {contract.agreement['unit_price']}, t = {contract.agreement['time']}")
            self.contracted_sales_at_t[contract.annotation['cfp'].product][contract.agreement['time']] += contract.agreement['quantity']
    ...
    def confirm_contract_execution(self, contract: Contract):
        ...
        if contract.annotation['buyer'] == self.id:
            self.executed_buys_at_t[contract.annotation['cfp'].product][self.awi.current_step] = (
                self.executed_buys_at_t[contract.annotation['cfp'].product][self.awi.current_step][0] + contract.agreement['quantity'],
                self.executed_buys_at_t[contract.annotation['cfp'].product][self.awi.current_step][1] + contract.agreement['unit_price'])
        else:
            self.executed_sales_at_t[contract.annotation['cfp'].product][self.awi.current_step] = (
                self.executed_sales_at_t[contract.annotation['cfp'].product][self.awi.current_step][0] + contract.agreement['quantity'],
                self.executed_sales_at_t[contract.annotation['cfp'].product][self.awi.current_step][1] + contract.agreement['unit_price'])
        return True

    def on_new_cfp(self, cfp: "CFP"):
        """Call whenever a CFP is posted. """
        if cfp.publisher != self.id:
            self.request_negotiation(cfp=cfp,
                                     negotiator=AspirationNegotiator(ufun=self.output_negotiator_ufun if cfp.is_buy else self.input_negotiator_ufun,
                                                                     aspiration_type=self.agent_aspiration_type))
    ...
    def print_debug_info(self, plan_for_inputs, number_of_inputs, number_of_outputs):
        ...
        table.add_row(['W'] + [round(money, 2) for money in self.wallet_history[start:]])
        for p in range(0, self.num_intermediate_products + 2):
            table.add_row([f'ST_{p}' + (str('*') if p == self.input_index or p == self.output_index else str('_'))] + self.storage_history[p][start:])
        table.add_row(['-----'] + ['-----' for t in range(start, end)])
        for p in range(0, self.num_intermediate_products + 2):
            table.add_row([f'CO_{p}' + (str('*') if p == self.input_index or p == self.output_index else str('_'))] +
                          [(self.contracted_buys_at_t[p][t], self.contracted_sales_at_t[p][t]) for t in range(start, end)])
    ...
    def plot_results(self):
        ...
        for p in range(0, self.num_intermediate_products + 2):
            ...
            ax = plt.subplot(str(self.num_intermediate_products + 3) + "1" + str(counter))
            ax.set_ylabel("P" + str(p) + ("*" if p == self.input_index or p == self.output_index else ""))
