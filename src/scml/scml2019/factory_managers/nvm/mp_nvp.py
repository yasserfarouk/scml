...
def read_qtty_feasible_domain(the_number_of_periods: int, the_quantities_domain_size: int) -> pd.DataFrame:
    ...
    :return:
    """
    return pd.read_csv('/'.join(__file__.split('/')[:-1]) + '/qtty_domain/qtty_domain_t_' + str(the_number_of_periods) + '_d_' + str(the_quantities_domain_size) + '.zip',
...
def pandas_tuple_to_list_of_tuple(sol: pd, number_of_periods: int) -> list:
    ...
    :return:
    """
    return [tuple((sol[t * 3], sol[t * 3 + 1], sol[t * 3 + 2])) for t in range(number_of_periods)] if sol is not None else None
